...
export class MealService {
  ...
  protected convertDateFromClient(meal: IMeal): IMeal {
    const copy: IMeal = Object.assign({}, meal, {
      when: meal.when && meal.when.isValid() ? meal.when.toJSON() : undefined
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.when = res.body.when ? moment(res.body.when) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((meal: IMeal) => {
        meal.when = meal.when ? moment(meal.when) : undefined;
      });
    }
    return res;
  }
